...
int dialog(Table* tab){
	...
	if (c==0)
		...
		if (c==2){
			...
			check_int (&n);
			if (n==0)
				find_item_fullkey(tab);
			if (n==1)
				find_item_key1(tab);
			if (n==2)
				find_item_key2(tab);
			if (n==3)
				find_key1_release(tab);
			if (n==4)
				find_key2_release(tab);
		}
		if (c==3){
...
void add(Table* tab){
	...
	scanf ("%s", info);
	insert_item (tab, info, key1, key2);
}
...
void insert_item(Table* tab, char* inf, char* key1, int key2){
...
void show_table(Table* tab){
	...
	while (tmp1 != NULL){
		...
		tmp2 = tab->ks2[k];
		while (strcmp(tmp1->info->info, tmp2->info->info) != 0)
			tmp2 = tmp2->next;
		i2 = tmp2->release+1;
				printf ("%s(%d), %d(%d), %s\n", tmp1->key1, i1, tmp2->key2, i2, tmp2->info->info);
		tmp1 = tmp1->next;
	}
}
...
void find_item_key1 (Table* tab){
	...
	while (n==0){
		...
		while (tmp != NULL){
			if (strcmp(tmp->key1,key1) == 0){
				printf ("%s(%d) {%s}\n", tmp->key1, tmp->release, tmp->info->info);
				n++;
			}
			tmp = tmp->next; 
		}
...
void find_item_key2 (Table* tab){
	...
	while (n==0){
		...
		int k = hash(key2);
			...
			while (tmp != NULL){
				if (tmp->key2 == key2){
					printf ("%d(%d) {%s}\n", tmp->key2, tmp->release, tmp->info->info);
					n++;
				}
				tmp = tmp->next; 
			}
		...
		if (search_release1(tab, key1) == 0)
			...
			while (tmp1 != NULL){
				if (strcmp(tmp1->key1,key1) == 0){
					...
					c = 0;
					while ((tmp2 != NULL)&&(c == 0)){
						if (strcmp(tmp1->info->info,tmp2->info->info) == 0){
							del_key1 (tab, i);
							del_key2 (tab, j, k);
							n++;
							c++;
		...
		while (tmp2 != NULL){
			...
			while (tmp22 != NULL){
				if (tmp2->key2 == tmp22->key2){
					...
					int c = 0;
					while ((tmp1 != NULL)&&(c == 0)){
						if (strcmp(tmp1->info->info,tmp22->info->info) == 0){
							del_key1 (tab, i);
							del_key2 (tab, j, k);
							i--;
							j--;
